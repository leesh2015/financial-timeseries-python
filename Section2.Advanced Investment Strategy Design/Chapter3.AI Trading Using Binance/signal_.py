"""
Trading Signal Generation with VECM-EGARCH Hybrid Model for Binance

This script generates trading signals using VECM (Vector Error Correction Model)
and EGARCH (Exponential GARCH) hybrid approach. The signals are saved to JSON
for execution by the Binance order system.
...
Trading Strategy:
- Long: When hybrid_yhat > actual_price and lower_price < lower_bound_mean
- Short: When hybrid_yhat < actual_price and upper_price > upper_bound_mean
...
def update_latest_data(df, tickers):
    """
    Update the latest date's data for all tickers using yf.Ticker.
    
    This function fetches the most recent market data to ensure
    the simulation uses up-to-date information.
    """
    last_date = df.index[-1]
    print(f"\nUpdating data for {last_date}")
    ...
    for ticker in tickers:
        try:
            ...
            df.loc[last_date, ('Open', ticker)] = ticker_info.get('regularMarketOpen', df.loc[last_date, ('Open', ticker)])
            df.loc[last_date, ('High', ticker)] = ticker_info.get('regularMarketDayHigh', df.loc[last_date, ('High', ticker)])
            df.loc[last_date, ('Low', ticker)] = ticker_info.get('regularMarketDayLow', df.loc[last_date, ('Low', ticker)])
            df.loc[last_date, ('Close', ticker)] = ticker_info.get('regularMarketPrice', df.loc[last_date, ('Close', ticker)])
            df.loc[last_date, ('Adj Close', ticker)] = ticker_info.get('regularMarketPrice', df.loc[last_date, ('Adj Close', ticker)])
            df.loc[last_date, ('Volume', ticker)] = ticker_info.get('regularMarketVolume', df.loc[last_date, ('Volume', ticker)])
            
            print(f"Updated {ticker} Open: {df.loc[last_date, ('Open', ticker)]:.2f}")
            if len(df) > 1:
                print(f"Previous {ticker} Open: {df.loc[last_date-pd.Timedelta(days=1), ('Open', ticker)]:.2f}")
...
def main():
    ...
    # Update latest data
    df = update_latest_data(df, TICKERS)
    ...
    if trade_history:
        ...
        output_file = os.path.join(script_dir, 'trade_history.xlsx')
    ...
    total_return = (metrics['final_capital'] - INITIAL_CAPITAL) / INITIAL_CAPITAL
    annualized_return = (1 + total_return) ** (1 / simulation_years) - 1
    print(f"Annualized Expected Return: {annualized_return:.2%}")
    
    # Get final position information
    final_position = {
        "date": test_data.index[-1].strftime('%Y-%m-%d'),
        "symbol": TARGET_INDEX,
        "signal": position if position else "none",  # Current position (long, short, or none)
        "entry_price": float(average_price) if position else 0,  # Average entry price
        "current_price": float(test_data[TARGET_INDEX].iloc[-1]),  # Latest price
        "exit_price": {
            "long": float(hybrid_yhat) if position == "long" else 0,  # Target exit price for long
            "short": float(hybrid_yhat) if position == "short" else 0  # Target exit price for short
        },
        "position_size": float(total_shares) if position else 0,  # Current position size
        "unrealized_pnl": float(unrealized_pnl),  # Unrealized profit/loss
        "forecast": {
            "predicted_price": float(hybrid_yhat),
    ...
    # Save to JSON file
    script_dir = os.path.dirname(os.path.abspath(__file__))
    json_file = os.path.join(script_dir, 'final_position.json')
    with open(json_file, 'w') as f:
        json.dump(final_position, f, indent=4)

    print(f"\nFinal position saved to '{json_file}'")
    print(f"Final position: {json.dumps(final_position, indent=2)}")
