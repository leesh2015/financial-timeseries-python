"""
Backtest for Chapter 2: Custom Kalman (Dynamic Beta)
Signals are generated from the custom alpha/beta estimates.
"""
...
from kalman_filter import estimate_alpha_beta_custom  # noqa: E402
...
def generate_beta_signals(
    ...
    # Custom Kalman uses fixed Q/R, so beta stays very smooth around the 3x anchor.
    # We therefore use relatively tight bands (default 10th/90th pct) to pick up small drift regimes.
...
def run_backtest_kalman():
    print("=" * 60)
    print("Chapter 2: Custom Kalman Backtest (Dynamic Beta)")
    print("=" * 60)

    data = load_nasdaq_tqqq_data(start_date="2020-01-01")
    results_alpha_beta = estimate_alpha_beta_custom(data["nasdaq"]["Returns"], data["tqqq"]["Returns"])
